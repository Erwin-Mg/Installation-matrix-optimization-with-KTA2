...
switch axis
    case 1
        % 获取所有唯一的第 2、3、4、5 列组合
        D = sortrows(D, [4 5 2 3 1]);
        [unique_combinations, ~, group_indices] = unique(D(:, 2:5), 'rows');
    case 2
        % 获取所有唯一的第 1、3、4、5 列组合
        D = sortrows(D, [4 5 1 3 2]);
        [unique_combinations, ~, group_indices] = unique(D(:, [1 3 4 5]), 'rows');
    case 3
        % 获取所有唯一的第 1、2、4、5 列组合
        D = sortrows(D, [4 5 1 2 3]);
        [unique_combinations, ~, group_indices] = unique(D(:, [1 2 4 5]), 'rows');
end
% 初始化结果存储
...
for i = 1:num_groups
    ...
    % 获取当前分组的第二列元素
    col_elements = D(current_group_indices, 2);
    % 检查第二列是否存在连续递增序列
    ...
    if length(col_elements) > 1
        for j = 2:length(col_elements)
            ...
            elseif ff  % 连续序列未至最后一项且连续序列中止
                ...
                if current_seq_length >max_seq_length           % 计算最长序列长度
                    max_seq_length = current_seq_length;        %长度
                    max_length = Length{length_num}.length ;    %起止行数
                end
                ff = false;
