...
% DACEFIT 根据所提供的数据集和回归模型对给定相关模型进行受限非线性最小二乘拟合
...
%
% Input
% S, Y    : Data points (S(i,:), Y(i,:)), i = 1,...,m   S个体变量,Y对应函数值
...
%    beta   : generalized least squares estimate 最小二乘估计
%    gamma  : correlation factors
%    sigma2 : maximum likelihood estimate of the process variance 过程方差的最大似然估计值
...
% Normalize data  计算均值及方差，并标准化初始数据
mS = mean(S);   sS = std(S);
mY = mean(Y);   sY = std(Y);
...
% Regression matrix
    F       = feval(regr, S);  %回归矩阵F，用于计算协方差cov
    [mF,p]  = size(F);
if  mF ~= m
...
% Determine theta
if  nargin > 5  % 若输入lob及upb上下界约束，则执行约束下非线性优化
